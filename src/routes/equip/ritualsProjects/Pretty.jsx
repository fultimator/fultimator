...
function LabelValueText({ item, align = "center", bold = false }) {
  if (!isLabelValue(item)) {
    return (
      <Typography sx={{ textAlign: align, width: "100%", lineHeight: 1, margin: 0, fontWeight: bold ? "bold" : "normal" }}>
        {item || ""}
      </Typography>
    );
  }
    ...
    <ThemeProvider theme={theme}>
      <Card sx={{ width: "100%" }}>
        <div ref={ref}>
          <Stack>
            ...
            >
              <Grid sx={{ flex: "0 0 70px", minWidth: "70px", minHeight: "40px" }} />
            ...
            <Grid container sx={{ borderBottom: `1px solid ${theme.secondary}` }}>
              <Grid
                sx={{
                  flex: "0 0 70px",
                  width: "70px",
              ...
              <Grid container direction="column" sx={{ flex: 1, minWidth: 0 }}>
                ...
                >
                  ...
                  <Grid sx={{ display: "flex", alignItems: "center", justifyContent: "center", p: 0 }} size={1}>
                    {hasDisplayValue(row2Values?.[2]) && <Diamond color={theme.primary} />}
                  </Grid>
                  <Grid sx={{ display: "flex", alignItems: "center", p: 0 }} size={3}>
            ...
            {Array.isArray(row3Values) && row3Values.length > 0 && (
              ...
              >
                ...
                <Grid container sx={{ flex: 1, pl: 1 }}>
                  <Grid sx={{ display: "flex", alignItems: "center", p: 0 }} size={3}>
                    <LabelValueText item={row3Values[0] || ""} align="left" bold />
                  </Grid>
                  <Grid sx={{ display: "flex", alignItems: "center", justifyContent: "center", p: 0 }} size={1}>
                    {!!row3Values[0] && !!row3Values[1] && <Diamond color={theme.primary} />}
                  </Grid>
                  <Grid sx={{ display: "flex", alignItems: "center", p: 0 }} size={4}>
            ...
            {Array.isArray(row4Values) && row4Values.length > 0 && (
              ...
              >
                ...
                <Grid container sx={{ flex: 1, pl: 1 }}>
                  <Grid sx={{ display: "flex", alignItems: "center", p: 0 }} size={3}>
                    <LabelValueText item={row4Values[0] || ""} align="left" bold />
                  </Grid>
                  <Grid sx={{ display: "flex", alignItems: "center", justifyContent: "center", p: 0 }} size={1}>
                    {!!row4Values[0] && !!row4Values[1] && <Diamond color={theme.primary} />}
                  </Grid>
                  <Grid sx={{ display: "flex", alignItems: "center", p: 0 }} size={4}>
                ...
                >
                  {footerLeftItems.map((item, idx) => (
                    ...
                    >
                      ...
                      {idx < footerLeftItems.length - 1 && <Diamond color={theme.primary} />}
                    </Grid>
                  ))}
                </Grid>
              )}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "16px", marginTop: "8px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          <Export name={`${name}`} dataType={exportDataType} data={exportData} />
        </div>
        <FormControlLabel
...
export function RitualPretty({ ritual }) {
  ...
  return (
    <PreviewShell
      ...
      row1Values={[ritual.name || t("No Name"), `${ritual.pm}`, `${ritual.dl}`, `${ritual.clock}`]}
      row2Values={[
