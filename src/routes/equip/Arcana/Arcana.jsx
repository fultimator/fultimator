...
function Arcana({ variant = "equip" }) {
  ...
  const { handleFileUpload } = useUploadJSON((data) => {
    ...
    setFormState((prev) => ({
      ...
      domain: data.domain ?? (data.domainDesc ? t(data.domainDesc) : prev.domain),
      domainDesc: data.domainDesc ? t(data.domainDesc) : (data.domain ?? prev.domainDesc),
      merge: data.merge ? t(data.merge) : (data.mergeName ?? prev.merge),
      mergeDesc: data.mergeDesc ? t(data.mergeDesc) : (data.mergeBenefit ?? prev.mergeDesc),
      pulse: data.pulse ? t(data.pulse) : (data.pulseName ?? prev.pulse),
      pulseDesc: data.pulseDesc ? t(data.pulseDesc) : (data.pulseBenefit ?? prev.pulseDesc),
      dismiss: data.dismiss ? t(data.dismiss) : (data.dismissName ?? prev.dismiss),
      dismissDesc: data.dismissDesc ? t(data.dismissDesc) : (data.dismissBenefit ?? prev.dismissDesc),
      spellType: data.spellType ?? (data.rework ? "arcanist-rework" : "arcanist"),
  ...
  const handleArcanumSelected = (item) => {
    setFormState((prev) => ({
      ...
      domain: item.domain ?? (item.domainDesc ? t(item.domainDesc) : prev.domain),
      domainDesc: item.domainDesc ? t(item.domainDesc) : (item.domain ?? prev.domainDesc),
      merge: item.merge ? t(item.merge) : (item.mergeName ?? prev.merge),
      mergeDesc: item.mergeDesc ? t(item.mergeDesc) : (item.mergeBenefit ?? prev.mergeDesc),
      pulse: item.pulse ? t(item.pulse) : (item.pulseName ?? prev.pulse),
      pulseDesc: item.pulseDesc ? t(item.pulseDesc) : (item.pulseBenefit ?? prev.pulseDesc),
      dismiss: item.dismiss ? t(item.dismiss) : (item.dismissName ?? prev.dismiss),
      dismissDesc: item.dismissDesc ? t(item.dismissDesc) : (item.dismissBenefit ?? prev.dismissDesc),
      spellType: item.spellType ?? (item.rework ? "arcanist-rework" : "arcanist"),
      ...
      <Grid size={{ xs: 12, sm: 6 }}>
        ...
        >
          ...
          <Grid container spacing={2} sx={{ mb: 1 }}>
            <SchemaFieldRenderer
              config={playerSpellFieldConfig.filter((f) => f.key !== "spellType")}
              state={formState}
