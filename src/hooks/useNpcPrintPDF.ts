...
function rankLabel(npc: TypeNpc): string {
  switch (npc.rank) {
    ...
    case "champion1":  return "Champion (1)";
    case "champion2":  return "Champion (2)";
    case "champion3":  return "Champion (3)";
    case "champion4":  return "Champion (4)";
    case "champion5":  return "Champion (5)";
    case "champion6":  return "Champion (6)";
...
function formatNpcAccuracy(acc: any): string {
  if (!acc) return "";
  const a1 = String(acc.attr1 ?? "").toUpperCase().slice(0, 3);
  const a2 = String(acc.attr2 ?? "").toUpperCase().slice(0, 3);
  if (!a1 && !a2) return "";
  const mod = acc.value !== undefined ? ` [${acc.value >= 0 ? "+" : ""}${acc.value}]` : "";
  return `${a1}+${a2}${mod}`;
}
...
function formatSpell(spell: any): string {
  ...
  if (spell.maxTargets !== undefined)   parts.push(`Targets: ${spell.maxTargets}`);
...
function buildFieldMap(npc: TypeNpc): Record<string, string> {
  ...
  for (const [type, fieldId] of Object.entries(AFFINITY_FIELDS)) {
    const val = String((npc.affinities as Record<string, string> ?? {})[type] ?? "");
  ...
  if (npc.armor?.name) {
    ...
    if (npc.armor.mdef !== undefined) armorParts.push(`M.DEF +${npc.armor.mdef}`);
    if (npc.armor.init !== undefined && npc.armor.init !== 0) armorParts.push(`INIT ${npc.armor.init}`);
    equipParts.push(armorParts.join(" "));
  }
  if (npc.shield?.name) {
    const shieldParts = [npc.shield.name];
    if (npc.shield.def  !== undefined) shieldParts.push(`DEF +${npc.shield.def}`);
    if (npc.shield.mdef !== undefined) shieldParts.push(`M.DEF +${npc.shield.mdef}`);
  ...
  if (melee.length)  fields["Campo testo 102020"] = melee.map(formatAttack).join("\n");
  if (ranged.length) fields["Campo testo 102021"] = ranged.map(formatAttack).join("\n");

  if (npc.spells?.length)  fields["Campo testo 102022"] = npc.spells.map(formatSpell).join("\n");
  if (npc.actions?.length) fields["Campo testo 102023"] = npc.actions.map(formatAction).join("\n");

  const specialLines = (npc.special ?? []).map(formatAction);
  const noteLines    = (npc.notes ?? []).map((n) => n.name ? `${n.name}: ${stripMarkdown(n.description ?? n.effect ?? "")}` : stripMarkdown(n.description ?? n.effect ?? "")).filter(Boolean);
