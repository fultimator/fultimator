...
type LegacySkill = { skillName?: string; name: string; currentLvl: number; maxLvl: number; description: string };
type LegacyNamed = { skillName?: string; name: string; description: string };

// Page 1: first 3 classes
const CLASS_FIELDS_P1 = [
  { classe: "Campo testo 285", benefici: "Campo testo 286", info: "Campo testo 10208" },
  { classe: "Campo testo 287", benefici: "Campo testo 288", info: "Campo testo 10209" },
  { classe: "Campo testo 289", benefici: "Campo testo 290", info: "Campo testo 102010" },
];

// Page 2: classes 4-7
const CLASS_FIELDS_P2 = [
  { classe: "Campo testo 253", benefici: "Campo testo 254", info: "Campo testo 10266" },
  { classe: "Campo testo 255", benefici: "Campo testo 256", info: "Campo testo 10267" },
  { classe: "Campo testo 257", benefici: "Campo testo 258", info: "Campo testo 10268" },
  { classe: "Campo testo 259", benefici: "Campo testo 260", info: "Campo testo 10269" },
];
...
const SPELL_FIELDS = [
  { name: "Campo testo 243",   mp: "Campo testo 244",   targets: "Campo testo 245",   duration: "Campo testo 246",   desc: "Campo testo 242"   },
  { name: "Campo testo 10237", mp: "Campo testo 10238", targets: "Campo testo 10239", duration: "Campo testo 10240", desc: "Campo testo 10236" },
  { name: "Campo testo 10242", mp: "Campo testo 10243", targets: "Campo testo 10244", duration: "Campo testo 10245", desc: "Campo testo 10241" },
  { name: "Campo testo 10247", mp: "Campo testo 10248", targets: "Campo testo 10249", duration: "Campo testo 10250", desc: "Campo testo 10246" },
  { name: "Campo testo 10252", mp: "Campo testo 10253", targets: "Campo testo 10254", duration: "Campo testo 10255", desc: "Campo testo 10251" },
  { name: "Campo testo 10257", mp: "Campo testo 10258", targets: "Campo testo 10259", duration: "Campo testo 10260", desc: "Campo testo 10256" },
  { name: "Campo testo 10262", mp: "Campo testo 10263", targets: "Campo testo 10264", duration: "Campo testo 10265", desc: "Campo testo 10261" },
  { name: "Campo testo 238",   mp: "Campo testo 239",   targets: "Campo testo 240",   duration: "Campo testo 241",   desc: "Campo testo 237"   },
  { name: "Campo testo 10212", mp: "Campo testo 10213", targets: "Campo testo 10214", duration: "Campo testo 10215", desc: "Campo testo 10211" },
  { name: "Campo testo 10217", mp: "Campo testo 10218", targets: "Campo testo 10219", duration: "Campo testo 10220", desc: "Campo testo 10216" },
  { name: "Campo testo 10222", mp: "Campo testo 10223", targets: "Campo testo 10224", duration: "Campo testo 10225", desc: "Campo testo 10221" },
  { name: "Campo testo 10227", mp: "Campo testo 10228", targets: "Campo testo 10229", duration: "Campo testo 10230", desc: "Campo testo 10226" },
  { name: "Campo testo 10232", mp: "Campo testo 10233", targets: "Campo testo 10234", duration: "Campo testo 10235", desc: "Campo testo 10231" },
];

// helpers

type DefItem = { modifiers?: { def?: number; mdef?: number; init?: number }; defModifier?: number; mDefModifier?: number; initModifier?: number };

const modDef  = (item: DefItem | null | undefined) => item?.modifiers?.def  ?? item?.defModifier  ?? 0;
const modMDef = (item: DefItem | null | undefined) => item?.modifiers?.mdef ?? item?.mDefModifier ?? 0;
const modInit = (item: DefItem | null | undefined) => item?.modifiers?.init ?? item?.initModifier ?? 0;
...
function buildClassSkillInfo(cls: TypePlayer["classes"][0]): { benefici: string; info: string } {
  const ben = cls.benefits as { hpplus?: number; mpplus?: number; ipplus?: number; other?: { description?: string }[] };
  ...
  (ben?.other ?? []).forEach((o) => { if (o.description) bonusParts.push(tr(o.description)); });

  const skillLines = (Array.isArray(cls.skills) ? cls.skills as LegacySkill[] : [])
...
function buildSpecialSpellBlock(spell: any): string[] {
  ...
  const customOrTr = (obj: { name?: string; customName?: string }, sentinel: string) =>
  ...
  switch (type) {
    case "pilot-vehicle":
      for (const v of (s.vehicles ?? [])) {
        ...
        const frame = (v.modules ?? []).find((m: { type?: string }) => m.type === "pilot_module_frame" || m.type?.includes("frame"));
        if (frame) lines.push(`  Frame: ${tr(frame.name)}${frame.description ? ` - ${tr(frame.description)}` : ""}`);
        for (const w of (v.modules ?? []).filter((m: { type?: string }) => m.type === "pilot_module_weapon" || m.type?.includes("weapon"))) {
          const acc = w.accuracy
            ...
            : "";
          const dmg = w.damage ? `${w.damage.value ?? ""} ${w.damage.type ?? ""}`.trim() : "";
          lines.push(`  Weapon: ${tr(w.name)}${acc ? ` ACC:${acc}` : ""}${dmg ? ` DMG:${dmg}` : ""}${w.description ? ` - ${tr(w.description)}` : ""}`);
        }
        const armor = (v.modules ?? []).find((m: { type?: string }) => m.type === "pilot_module_armor" || m.type?.includes("armor"));
        if (armor) lines.push(`  Armor: ${tr(armor.name)} DEF+${armor.def ?? 0} MDEF+${armor.mdef ?? 0}${armor.description ? ` - ${tr(armor.description)}` : ""}`);
        for (const sup of (v.modules ?? []).filter((m: { type?: string }) => m.type === "pilot_module_support" || m.type?.includes("support")))
          lines.push(`  Support: ${tr(sup.name)}${sup.description ? ` - ${tr(sup.description)}` : ""}`);
      }
      break;
    case "dance":
      lines.push(`[DANCER]`);
      for (const d of (s.dances ?? [])) {
        ...
        const dur  = d.name === "dance_custom_name" ? (d.duration ?? "") : tr(d.duration ?? "");
        const eff  = d.name === "dance_custom_name" ? (d.effect ?? "")   : tr(d.effect ?? "");
        lines.push(`  ${name}${dur ? ` (${dur})` : ""}${eff ? ` - ${stripMarkdown(eff)}` : ""}`);
      }
      break;
    case "symbol":
      ...
      for (const sym of (s.symbols ?? [])) {
        ...
        const eff  = sym.name === "symbol_custom_name" ? (sym.effect ?? "") : tr(sym.effect ?? "");
    ...
    case "gift":
      ...
      for (const g of (s.gifts ?? []))
        lines.push(`  ${tr(g.name ?? "")}${g.effect || g.description ? ` - ${stripMarkdown(tr(g.effect ?? g.description ?? ""))}` : ""}`);
      break;
    case "therioform":
      lines.push(`[MUTANT]`);
      for (const f of (s.therioforms ?? [])) {
        const name = customOrTr(f, "mutant_therioform_custom_name");
        const desc = f.name === "mutant_therioform_custom_name" ? (f.description ?? "") : tr(f.description ?? "");
    ...
    case "magichant":
      ...
      for (const k of (s.keys ?? [])) {
        ...
        lines.push(`  Key: ${name}${k.mp !== undefined ? ` (MP ${k.mp})` : ""}`);
      }
      for (const tone of (s.tones ?? [])) {
        const name = customOrTr(tone, "magichant_custom_name");
        const eff  = tone.name === "magichant_custom_name" ? (tone.effect ?? "") : tr(tone.effect ?? "");
    ...
    case "invocation":
      ...
      for (const inv of (s.invocations ?? []))
        lines.push(`  ${tr(inv.name ?? "")}${inv.effect ? ` - ${stripMarkdown(tr(inv.effect))}` : ""}`);
      break;
    case "magiseed":
    ...
    case "tinkerer-magitech":
      ...
      for (const rank of (s.ranks ?? s.gadgets ?? []))
        lines.push(`  ${tr(rank.name ?? "")}${rank.effect || rank.description ? ` - ${stripMarkdown(tr(rank.effect ?? rank.description ?? ""))}` : ""}`);
      break;
    case "gamble":
      lines.push(`[ENTROPIST] ${s.spellName ?? tr(s.name ?? "")}`);
      if (s.mp) lines.push(`  MP: ${s.mp}`);
      for (const target of (s.targets ?? [])) {
        const effects = (target.effects ?? []).map((e: { effect?: string }) => stripMarkdown(tr(e.effect ?? ""))).filter(Boolean).join("; ");
        lines.push(`  Target: ${target.effect ?? ""}${effects ? ` [${effects}]` : ""}`);
      }
      break;
    case "deck":
...
function buildFieldMap(player: TypePlayer): Record<string, string | boolean> {
  ...
  const currIns = calculateAttribute(player, player.attributes?.insight?.base,   ["dazed",  "enraged"],  ["insUp"], 6, 12);
  const currMig = calculateAttribute(player, player.attributes?.might?.base,     ["weak",   "poisoned"], ["migUp"], 6, 12);
  const currWlp = calculateAttribute(player, player.attributes?.willpower?.base, ["shaken", "poisoned"], ["wlpUp"], 6, 12);
  ...
  const stats = player.stats as { hp: RuntimeStat; mp: RuntimeStat; ip: RuntimeStat } | undefined;
  ...
  const pilotSpells  = (player.classes ?? []).flatMap((c) => Array.isArray(c.spells) ? c.spells : []).filter((s) => s?.spellType === "pilot-vehicle");
  const activeVehicle = pilotSpells.flatMap((s) => s.vehicles ?? []).find((v) => v.enabled);
  type ArmorModule = VehicleModule & { martial?: boolean; def?: number; mdef?: number };
  const armorModule = (activeVehicle?.modules?.find((m) => m.equipped && m.type === "pilot_module_armor") ?? null) as ArmorModule | null;

  const isMartialArmor = armorModule ? armorModule.martial : (equippedArmor?.martial ?? false);
  const dodgeBonus =
    equippedShields.length === 0 && !isMartialArmor
      ? (player.classes ?? []).flatMap((c) => Array.isArray(c.skills) ? c.skills : []).filter((s) => s.specialSkill === "Dodge").reduce((sum, s) => sum + (s.currentLvl || 0), 0)
      : 0;

  const baseDef = armorModule
    ? (armorModule.martial ? armorModule.def || 0 : currDex + (armorModule.def || 0))
    : equippedArmor
      ? (equippedArmor.martial ? equippedArmor.def : currDex + equippedArmor.def)
      : currDex;
  ...
  const currDef =
    ...
    equippedCustomWeapons.reduce((a, w: CustomWeapons) => a + (parseInt(String(modDef(w)), 10) || 0), 0) +
    dodgeBonus;

  const baseMDef = armorModule
    ? (armorModule.martial ? armorModule.mdef || 0 : currIns + (armorModule.mdef || 0))
    : equippedArmor ? currIns + equippedArmor.mdef : currIns;
  ...
  const currMDef =
    ...
    equippedCustomWeapons.reduce((a, w: CustomWeapons) => a + (parseInt(String(modMDef(w)), 10) || 0), 0);
  ...
  const findEquipItem = (ref: typeof player.equippedSlots.mainHand) => {
    ...
    const arr = inv[ref.source as keyof typeof inv] as { name: string }[] | undefined;
    if (!arr) return null;
  ...
  if (mainHand)    { fields["Mano1Equip"] = mainHand.name; fields["Mano1EquipDesc"] = buildWeaponDesc(mainHand as Weapons); }
  if (offHand)     { fields["Mano2Equip"] = offHand.name;  fields["Mano2EquipDesc"] = buildWeaponDesc(offHand as Weapons); }
  if (armorItem)   { fields["ArmaturaEquip"] = armorItem.name; fields["ArmaturaEquipDesc"] = buildArmorDesc(armorItem as Armor); }
  if (accessoryItem) { fields["AccessorioEquip"] = accessoryItem.name; fields["AccessorioEquipDesc"] = stripMarkdown((accessoryItem as Accessories).description ?? ""); }
  ...
  const heroics = (player.classes ?? []).flatMap((cls) =>
    (Array.isArray(cls.heroic) ? cls.heroic as LegacyNamed[] : [])
      ...
      .map((h) => h.description ? `${tr(h.skillName ?? h.name)} - ${tr(h.description)}` : tr(h.skillName ?? h.name))
  );
  if (heroics.length) fields["Campo testo 10270"] = heroics.join("\n");

  // Spells
  const STANDARD_SPELL_TYPES = new Set(["default", "arcane", "ritual", "gamble", "arcanist", "arcanist-rework", undefined, null, ""]);
  const allSpells    = (player.classes ?? []).flatMap((cls) => Array.isArray(cls.spells) ? cls.spells : []);
  const tableSpells  = allSpells.filter((s) => STANDARD_SPELL_TYPES.has(s.spellType ?? ""));
  const specialSpells = allSpells.filter((s) => !STANDARD_SPELL_TYPES.has(s.spellType ?? ""));
  ...
  tableSpells.slice(0, SPELL_FIELDS.length).forEach((spell, i) => {
    ...
    const effects = [spell.effect1, spell.effect2, spell.effect3, spell.effect4, spell.effect5, spell.effect6]
      .filter(Boolean).map((e) => tr(e!)).join(" ");
    fields[f.desc] = effects || tr(spell.description ?? "");
  });

  const specialBlocks = specialSpells.map(buildSpecialSpellBlock).filter((b) => b.length).map((b) => b.join("\n"));
  if (specialBlocks.length) fields["Campo testo 10317"] = specialBlocks.join("\n\n");
  ...
  if (ritualSpells.length) fields["Campo testo 10210"] = ritualSpells.map((s) => tr(s.name ?? "")).join(", ");

  // Notes / inventory
  const inventoryLines  = (player.items ?? []).map((item) => `${item.name} x${item.quantity}${item.description ? ` - ${item.description}` : ""}`);
  const consumableLines = (player.consumables ?? []).map((c) => `${c.name} (IP ${c.ipCost})${c.description ? ` - ${c.description}` : ""}`);
  const noteLines       = (player.notes ?? []).map((n) => n.name ? `${n.name}: ${n.description}` : n.description);

  if (inventoryLines.length || consumableLines.length) fields["ZainoAppunti 2"] = [...inventoryLines, ...consumableLines].join("\n");
...
function computePageBreaks(element: HTMLElement, scale: number, pageHeightPx: number, totalHeightPx: number): number[] {
  ...
  while (cursor + pageHeightPx < totalHeightPx) {
    ...
    const culprits = sections.filter(
      (s) => s.top < breakY && s.bottom > breakY && s.bottom - s.top < pageHeightPx
    );
...
export async function buildAppPDF(canvas: HTMLCanvasElement, element: HTMLElement, scale: number, filename: string): Promise<void> {
  const contentW = A4_W - PDF_MARGIN * 2;
  const pageHeightPx = Math.round((canvas.width * (A4_H - PDF_MARGIN * 2)) / (A4_W - PDF_MARGIN * 2));
  const breakPoints  = computePageBreaks(element, scale, pageHeightPx, canvas.height);
  ...
  for (let i = 0; i < boundaries.length - 1; i++) {
    ...
    page.drawImage(image, { x: PDF_MARGIN, y: PDF_MARGIN, width: contentW, height: contentH });
  }
...
export default function usePrintPDF() {
  const printPDF = async (player: TypePlayer) => {
    const response = await fetch("/pdf/Fabula-Ultima-Sheets-Quirks-Optional.pdf");
    const pdf      = await PDF.load(new Uint8Array(await response.arrayBuffer()));
    const form     = pdf.getForm();
    if (!form) { console.warn("No form found in PDF"); return; }
    form.fill(buildFieldMap(player));
    triggerDownload(await pdf.save(), `${player.name ?? "character"}_print.pdf`);
  };
