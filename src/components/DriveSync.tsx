...
export default function DriveSync() {
  ...
  const handleRestore = async () => {
    const confirmed = window.confirm(
      t("This will overwrite your local data with the Google Drive backup. Continue?")
    );
    if (!confirmed) return;
    setStatus("restoring");
    try {
  ...
  return (
    <>
      <Tooltip title={!signedIn ? t("Have a Cloud account? Sign in to enable Drive sync.") : ""}>
        <span>
          <ButtonGroup variant="outlined" size="small" disabled={!signedIn || busy}>
            <Tooltip title={signedIn ? (hasPending ? t("Unsaved changes - sync to Google Drive") : t("Sync local data to Google Drive")) : ""}>
              <span>
                <Button
                  startIcon={status === "syncing" ? <CircularProgress size={14} /> : <CloudUploadIcon />}
                  onClick={handleSync}
                  disabled={!signedIn || busy}
                  sx={signedIn && hasPending ? {
                    color: "#c9963a",
                    borderColor: "#c9963a",
            ...
            <Tooltip title={signedIn ? t("Restore local data from Google Drive backup") : ""}>
              <span>
                <Button
                  startIcon={status === "restoring" ? <CircularProgress size={14} /> : <CloudDownloadIcon />}
