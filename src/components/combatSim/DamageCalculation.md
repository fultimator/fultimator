...
- Does this attack Grant or Remove a modifier?
  - Yes:
    ...
    - If it's a Remove (without lock) check whether there's a remove lock on that affinity and type of damage - If there's a lock on that affinity → No change since it's already removed - If there's no lock on that affinity → Remove the selected VU / RS / IM / AB → Remove previous modifier if any
