...
export const WeaponCard = React.memo(function WeaponCard({
  weapon,
  id,
  onHeaderClick,
}) {
...
export const ArmorCard = React.memo(function ArmorCard({
  armor,
  id,
  onHeaderClick,
}) {
...
export const PlayerSpellCard = React.memo(function PlayerSpellCard({
  spell,
  id,
  onHeaderClick,
}) {
          ...
          >
            {spell.isOffensive && attr1 && attr2 && (
              <strong>
                ...
                <CloseBracket /> <Diamond /> <OpenBracket />
                HR + {spell.damage || 0}
                <CloseBracket /> {spell.damageType || "physical"}{" "}
                <Diamond />{" "}
              </strong>
            )}
            <span style={{ display: "inline" }}>
...
export const NonStaticSpellCard = React.memo(function NonStaticSpellCard({
  item,
  id,
  onHeaderClick,
}) {
  ...
  const pilotSubtype =
    ...
    (String(item.category || "")
      .toLowerCase()
      .includes("armor module")
      ? "armor"
      : null) ||
    (String(item.category || "")
      .toLowerCase()
      .includes("weapon module")
      ? "weapon"
      : null) ||
    (String(item.category || "")
      .toLowerCase()
      .includes("support module")
      ? "support"
      : null);
  ...
  const renderBody = () => {
    switch (item.spellType) {
      ...
      case "magichant":
        if (
          ...
          const keyDetails = [
            ...
            {
              label: t("Recovery"),
              value: item.recovery ? t(item.recovery) : "",
            },
      ...
      case "pilot-vehicle":
        if (pilotSubtype === "frame" || item.passengers != null)
          return (
            <>
              <Typography
                variant="caption"
              ...
              >
                {t("Passengers")}: {item.passengers ?? " - "} · {t("Distance")}:{" "}
                {item.distance ?? " - "}
              </Typography>
              {item.description && (
                <Typography
        ...
        if (pilotSubtype === "armor" || item.def != null)
          return (
            <>
              <Typography
                variant="caption"
        ...
        if (pilotSubtype === "weapon" || item.damage != null)
          return (
            <>
              <Typography
                variant="caption"
              ...
              >
                {[
                  item.category,
                  item.att1 && item.att2 ? `${item.att1}+${item.att2}` : null,
                ]
                  .filter(Boolean)
                  .join(" · ")}
              </Typography>
  ...
  return (
    <Card id={id} elevation={1}>
      <Stack>
        ...
        <Box sx={{ px: 1, py: 1 }}>{renderBody()}</Box>
      </Stack>
    </Card>
  );
});
...
export const AttackCard = React.memo(function AttackCard({
  attack,
  id,
  onHeaderClick,
}) {
      ...
      <Stack>
        ...
        {attack.special?.length > 0 && (
          <Grid container sx={{ px: 1, py: "4px" }}>
            <Grid size={12}>
              <Typography variant="body2">
                <strong>{t("Special")}:</strong> {attack.special.join("; ")}
              </Typography>
            </Grid>
          </Grid>
        )}
...
export const QualityCard = React.memo(function QualityCard({
  quality,
  id,
  onHeaderClick,
}) {
...
const _therioformItems = availableTherioforms.filter(
  (tf) => !tf.name.includes("_custom_"),
);
const _magichantKeyItems = availableMagichantKeys.filter(
  (key) => !key.name.includes("_custom_"),
);
const _magichantToneItems = availableMagichantTones.filter(
  (tone) => !tone.name.includes("_custom_"),
);
const _symbolItems = availableSymbols.filter(
  (s) => !s.name.includes("_custom_"),
);
...
const _pilotArmorModules = availableModules.armor.filter(
  (m) => m.name !== "pilot_custom_armor",
);
const _pilotWeaponModules = availableModules.weapon;
const _pilotSupportModules = availableModules.support.filter(
  (m) => m.name !== "pilot_custom_support",
);
...
function renderSpellTypeContent(sc, t, customTheme) {
  ...
  switch (sc) {
    ...
    case "magichant":
      return [
        <Box key="keys_h" sx={sectionHeaderSx}>
          {captionHeader(t("magichant_key"))}
        </Box>,
        ...
        <Box key="tones_h" sx={sectionHeaderSx}>
          {captionHeader(t("magichant_tone"))}
        </Box>,
    ...
    case "invocation":
      return _invocationsByWellspringEntries.flatMap(
        ([wellspring, invocations]) => [
          <Box key={wellspring + "_h"} sx={sectionHeaderSx}>
            {captionHeader(wellspring)}
          </Box>,
    ...
    case "tinkerer-alchemy":
      return [
        <Box key="targets_h" sx={sectionHeaderSx}>
          {captionHeader(t("Targets"))}
        </Box>,
        ...
        <Box key="effects_h" sx={sectionHeaderSx}>
          {captionHeader(t("Effects"))}
        </Box>,
    ...
    case "tinkerer-magitech":
      return [
        {
          rankLabel: t("Basic"),
          name: t("Magitech Override"),
          descKeys: ["MagitechOverride_desc"],
        },
        {
          rankLabel: t("Advanced"),
          name: t("Magicannon"),
          descKeys: ["Magicannon_desc1", "Magicannon_desc2"],
        },
        {
          rankLabel: t("Superior"),
          name: t("Magispheres"),
          descKeys: [
            "Magispheres_desc1",
            "Magispheres_desc2",
    ...
    case "pilot-vehicle":
      return [
        <Box key="frames_h" sx={sectionHeaderSx}>
          {captionHeader(t("Frames"))}
        </Box>,
        ...availableFrames.map((frame, i) => (
          <Box key={"fr" + i} sx={itemSx}>
            ...
            >
              ...
              >
                {t("Passengers")}: {frame.passengers} · {t("Distance")}:{" "}
                {frame.distance}
              </Typography>
            </Box>
            <Typography
            ...
            >
              {md(t(frame.description))}
            </Typography>
          </Box>
        )),

        <Box key="armor_h" sx={sectionHeaderSx}>
          {captionHeader(t("Armor Modules"))}
        </Box>,
        ...
        <Box key="weapon_h" sx={sectionHeaderSx}>
          {captionHeader(t("Weapon Modules"))}
        </Box>,
        ...
        <Box key="support_h" sx={sectionHeaderSx}>
          {captionHeader(t("Support Modules"))}
        </Box>,
...
export const ClassCard = React.memo(function ClassCard({
  ...
  const { t } = useTranslate();
  const customTheme = useCustomTheme();
  const [expandedSpellAccordion, setExpandedSpellAccordion] =
    React.useState(null);
          ...
          return (
            <Accordion
              ...
              onChange={(_, isExpanded) =>
                setExpandedSpellAccordion(isExpanded ? cls.name : null)
              }
              sx={{
                    ...
                    {hasCustomSpells
                      ? /* Per-character spell types */
                        cls.benefits.spellClasses.map((sc) => {
                          ...
                          const content = renderSpellTypeContent(
                            sc,
                            t,
                            customTheme,
                          );
...
export const SpecialRuleCard = React.memo(function SpecialRuleCard({
  item,
  id,
  onHeaderClick,
}) {
...
export const CustomWeaponCard = React.memo(function CustomWeaponCard({
  weapon,
  id,
  onHeaderClick,
}) {
...
export const AccessoryCard = React.memo(function AccessoryCard({
  accessory,
  id,
  onHeaderClick,
}) {
...
export const VehicleModuleCard = React.memo(function VehicleModuleCard({
  module,
  id,
  onHeaderClick,
}) {
...
export const HeroicCard = React.memo(function HeroicCard({
  heroic,
  id,
  onHeaderClick,
}) {
...
export const OptionalCard = React.memo(function OptionalCard({
  optional,
  id,
  onHeaderClick,
}) {
