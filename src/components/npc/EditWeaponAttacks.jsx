      ...
      >
        {weapons.map((w) => (
          <MenuItem key={w.name} value={w.name}>
            {w.name} {w.martial && <Martial />}{" "}
            <OpenBracket />
            {attributes[w.accuracy?.attr1]?.shortcaps}+{attributes[w.accuracy?.attr2]?.shortcaps}
...
export default function EditWeaponAttacks({ npc, setNpc }) {
  ...
  const removeAttack = (i) => {
    ...
    setNpc((prev) => ({
      ...prev,
      weaponattacks: (prev.weaponattacks || []).filter((_, index) => index !== i),
    }));
  };
    ...
    <>
      ...
      {npc.weaponattacks?.map((attack, i) => {
        ...
        const handleRoll = (e) => {
          e.stopPropagation();
          const dieSizes = {
            primary: npc.attributes?.[attack.accuracy?.attr1]?.base ?? npc.attributes?.[attack.accuracy?.attr1] ?? 6,
            secondary: npc.attributes?.[attack.accuracy?.attr2]?.base ?? npc.attributes?.[attack.accuracy?.attr2] ?? 6,
          };
            ...
            <AccordionDetails>
              <Grid container spacing={1}>
                ...
                <Grid size={12}>
                  <SelectWeapon
                    attack={attack}
                    onChange={(fields) => updateAttack({ ...attack, ...fields })}
                  />
                </Grid>
                <SchemaFieldRenderer
      ...
      <DeleteConfirmationDialog
        ...
        itemPreview={pendingAttackIndex !== null ? npc.weaponattacks?.[pendingAttackIndex]?.name || "" : ""}
      />
    </>
  );
}
