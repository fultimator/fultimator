...
function ThemedSpellDeck({
  ...
  const deckState = useMemo(
    () => ({
      ...
      cardsInDeck:
        ...
        (deck?.cardsInDeck !== undefined ? deck.cardsInDeck : 30),
      hand: deck?.hand || [],
  ...
  const handleTrapCard = (suit) => {
    ...
    const logEntry = {
      ...
      effect:
        resultMessage +
        (success
          ? " Success! You may perform the Spell action for free (mp cost <= SL x 5)."
          : " Failure."),
    };
    setCombatLog((prev) => [logEntry, ...prev]);
  };
  ...
  const detectPossibleSets = (hand) => {
      ...
      if (cards.length >= 4 && suit !== "joker") {
        ...
        sequences.forEach((seq, index) => {
          ...
          allSuggestions.push({
            type: "Magic Flush",
            cards: seq,
            description: t("ace_magic_flush_enriched", [
              25 + totalValue,
              suit.toLowerCase(),
            ]),
            priority: 5,
    ...
    allSuggestions.forEach((suggestion) => {
      const cardIndices = suggestion.cards
        .map((c) => c.handIndex)
        .sort()
        .join("-");
  ...
  const resolveSet = () => {
    ...
    const newHand = deck.hand.filter(
  ...
  return (
    <>
        ...
        <AccordionDetails>
          <Box sx={{ p: 1 }}>
            ...
            <Box sx={{ mt: 3 }}>
              <Typography
                variant="h6"
                sx={{ mb: 2, fontWeight: "bold", color: theme.primary }}
              >
                {t("ace_details")}
              </Typography>
              ...
              <Typography
                variant="h6"
                sx={{ mb: 2, mt: 3, fontWeight: "bold", color: theme.primary }}
              >
            ...
            >
              {deckState.hand.map((card, index) => {
                ...
                const isHoveredPreview =
                  hoveredSuggestion &&
                  hoveredSuggestion.cards.some((c) => c.handIndex === index);
                  ...
                  <Box
                    ...
                    sx={{
                      ...
                      width: (() => {
                        ...
                        if (handSize <= 3)
                          return { xs: "28%", sm: "25%", md: "20%" };
                        if (handSize <= 5)
                          return { xs: "18%", sm: "18%", md: "16%" };
                        if (handSize <= 8)
                          return { xs: "15%", sm: "14%", md: "12%" };
                        if (handSize <= 12)
                          return { xs: "12%", sm: "11%", md: "10%" };
                        if (handSize <= 16)
                          return { xs: "11%", sm: "10%", md: "9%" };
                        if (handSize <= 20)
                          return { xs: "10%", sm: "9%", md: "8%" };
                    ...
                    <Card
                      ...
                      sx={{
                        ...
                        border: isSelected
                          ...
                          : isHoveredPreview
                            ? "2px solid #ffa726"
                            : "1px solid", // Selected: cyan, Preview: orange, Default: divider
                        ...
                        {suggestions.map((suggestion, index) => (
                          <Chip
                            ...
                            onClick={() => {
                              ...
                              const cardsToSelect = suggestion.cards.map(
                                (card) => ({
                                  ...card,
              ...
              {combatLog.map((entry, index) => (
                <Box
                  ...
                  sx={{
                    ...
                    py: 0.5,
                    borderBottom:
                      index < combatLog.length - 1 ? "1px solid" : "none",
      ...
      {deckState.hand.length === 0 && (
        <Box
          sx={{ padding: 2, backgroundColor: theme.ternary, marginBottom: 2 }}
        >
                      ...
                      >
                        ...
                        <Box
                          sx={{
                            ...
                            right: 2,
                            backgroundColor:
                              index === 0 ? "#22c55e" : "#3b82f6", // Explicit bright colors
