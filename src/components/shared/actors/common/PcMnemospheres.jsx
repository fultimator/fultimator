              ...
              <Tooltip title={t("Increase Level")}>
                <span>
                  <IconButton
                    ...
                    onClick={onIncrease}
                    disabled={
                      skill.currentLvl >= skill.maxLvl || budgetExhausted
                    }
                  >
...
function MnemoCard({
  ...
  const skills = (mnemo.skills ?? []).filter(
    (s) => isInteractive || (s.currentLvl ?? 0) >= 1,
  );
  ...
  const filteredSkills = skills.filter((skill) => {
    ...
    return (
      t(skill.skillName ?? skill.name ?? "")
        .toLowerCase()
        .includes(q) || desc.toLowerCase().includes(q)
    );
  });
                  ...
                  <span>
                    <IconButton
                      ...
                      disabled={
                        !onInvestLevel ||
                        sphereLvl >= 5 ||
                        (availableLevels != null && availableLevels <= 0)
                      }
                    >
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Typography sx={{ fontWeight: "bold", fontSize: "0.85rem", flex: 1 }}>
            {highlightMatch(t(mnemo.class ?? mnemo.name ?? ""), searchQuery)}
          </Typography>
          <Box
            sx={{
        ...
        {filteredSkills.map((skill, skillIdx) => {
          ...
          return (
            <ItemRowCard
              ...
              onCardClick={
                desc
                  ? () => setPreview({ type: "skill", skill, desc })
                  : undefined
              }
              paperSx={{
        ...
        {filteredHeroics.map((h, hIdx) => {
          ...
          return (
            <ItemRowCard
              ...
              onCardClick={
                desc
                  ? () => setPreview({ type: "heroic", heroic: h, desc })
                  : undefined
              }
              paperSx={{
                gridColumn: "1 / -1",
                ...
                desc ? (
                  <Tooltip title={t("Send to Chat")}>
                    <IconButton
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        sendDisplayMessage("heroic skill", t(h.name ?? ""), {
                          speaker: pc?.info?.name || pc?.name || "",
                          description: desc || undefined,
                        });
                      }}
      ...
      >
        <DialogContent sx={{ p: 0 }}>
          {preview?.type === "skill" && (
            <SharedSkillCard
              item={{
                skillName: preview.skill.skillName ?? preview.skill.name ?? "",
                description: preview.skill.description,
                currentLvl: preview.skill.currentLvl,
                className: mnemo.class ?? mnemo.name ?? "",
              }}
            />
          )}
          {preview?.type === "heroic" && (
...
export default function PcMnemospheres({
  ...
  const breakdown = bankSpheres
    .map((m) => ({
      ...
      label: `${t(m.class ?? m.name ?? "?")} Lv.${m.lvl ?? 1}`,
    }))
  ...
  const actions = (
    ...
    >
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: "2px" }}>
              {[
                ...
                usesInnateClassRules
                  ? characterLevel - totalInnateInvested
                  : characterLevel,
              ].map((val, i) => (
      ...
      <Tooltip
        title={expandSignal?.expanded ? t("Collapse All") : t("Expand All")}
      >
        <IconButton
          component="span"
          size="small"
          data-expand-all-mnemo={
  ...
  const content = (
    <Box sx={{ p: isCompact ? "4px" : 1 }}>
      {slottedMnemospheres.map((mnemo) => {
        ...
        const availableLevels = getMnemoAvailableLevels
          ? getMnemoAvailableLevels(mnemo)
          : null;
        ...
        return (
          <MnemoCard
            ...
            onIncreaseSkillLevel={(skillIdx) =>
              onChangeMnemoSkillLevel?.(mnemo.id, skillIdx, 1)
            }
            onDecreaseSkillLevel={(skillIdx) =>
              onChangeMnemoSkillLevel?.(mnemo.id, skillIdx, -1)
            }
            onInvestLevel={
              isInteractive ? () => onInvestLevel?.(mnemo.id) : null
            }
            onRefundLevel={
              isInteractive ? () => onRefundLevel?.(mnemo.id) : null
            }
  ...
  return isCompact ? (
    <Paper sx={{ mb: 1, overflow: "hidden" }} elevation={0} variant="outlined">
      <CompactSectionHeader title={t("Mnemospheres")}>
        {actions}
