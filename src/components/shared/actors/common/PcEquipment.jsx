...
function CompactItemRow({
  ...
  const sendToChatAction = () => {
    ...
    sendDisplayMessage("item", t(item.name), {
      ...
      tags,
      description: item.quality || item.description || undefined,
    });
  };
    ...
    <>
      <ItemRowCard
        ...
        actions={
          <>
            {isWeapon ? (
              <IconButton size="small" onClick={() => handleDiceRoll(item)}>
                <Casino />
              </IconButton>
            ) : (
              <Tooltip title={t("Send to Chat")} arrow>
                <IconButton size="small" onClick={sendToChatAction}>
                  <Message />
                </IconButton>
              </Tooltip>
            )}
    ...
    <>
      <ItemRowCard
        ...
        actions={
          isActive ? (
            <>
              <IconButton size="small" onClick={() => onRoll(form)}>
                <Casino />
              </IconButton>
              <IconButton
                size="small"
            ...
            >
              {isEquipped ? (
                <MenuItem
                  disabled={!isEditMode}
                  onClick={() => {
                    onEquip(source, index, item, isTwoHand, slotLabel, null);
                ...
                [
                  <MenuItem
                    ...
                    disabled={!isEditMode}
                    onClick={() => {
                      onEquipToSlot?.(source, index, item, "mainHand");
                      setMenuAnchorEl(null);
                    }}
                  >
                    <RadioButtonUnchecked
                  ...
                  <MenuItem
                    ...
                    disabled={!isEditMode}
                    onClick={() => {
                      onEquipToSlot?.(source, index, item, "offHand");
                      setMenuAnchorEl(null);
                    }}
                  >
                    <RadioButtonUnchecked
                ...
                <MenuItem
                  disabled={!isEditMode}
                  onClick={(e) => {
                    onEquip(source, index, item, isTwoHand, slotLabel, e);
                    setMenuAnchorEl(null);
                  }}
                >
                  <RadioButtonUnchecked
              ...
              {onAddToCompendium && (
                <MenuItem
                  onClick={async () => {
                    await onAddToCompendium(source, item);
                    setMenuAnchorEl(null);
                  }}
                >
                  <AddToPhotosIcon
...
const FullTransformingPair = memo(function FullTransformingPair({
  ...
  const buildFormData = (cw, form) =>
    form === "primary"
      ? {
          name: cw.name,
          category: cw.category || "weapon_category_brawling",
          range: cw.range || "weapon_range_melee",
          accuracy: cw.accuracy,
          damage: cw.damage,
          equipType: "custom-weapon",
        }
      : {
          name: cw.secondWeaponName || `${cw.name} (Alt)`,
          category: cw.secondSelectedCategory || "weapon_category_brawling",
          range: cw.secondSelectedRange || "weapon_range_melee",
          accuracy: cw.secondAccuracy ?? cw.accuracy,
          damage: cw.secondDamage ?? cw.damage,
          equipType: "custom-weapon",
        };
          ...
          <Grid key={form} size={{ xs: 12, sm: 6 }}>
            ...
            >
              <ItemRowCard
                ...
                onCardClick={
                  !isActive ? () => onSwap(index) : () => onPreview?.(fd)
                }
                actions={
                  isActive ? (
                    <>
                      ...
                      >
                        ...
                        <MenuItem
                          disabled={!isEditMode}
                          onClick={() => {
                            onEdit("customWeapons", index, item);
                            setMenuAnchorEl(null);
                          }}
                        >
                          <Edit
                        ...
                        <MenuItem
                          disabled={!isEditMode}
                          onClick={() => {
                            onDelete("customWeapons", index, item);
                            setMenuAnchorEl(null);
                          }}
                          sx={{
                            "&:not(.Mui-disabled)": { color: "error.main" },
                          }}
                        >
                          <Delete
                            fontSize="small"
                        ...
                        {onAddToCompendium && (
                          <MenuItem
                            onClick={async () => {
                              await onAddToCompendium("customWeapons", item);
                              setMenuAnchorEl(null);
                            }}
                          >
                            <AddToPhotosIcon
...
function resolveSlotLabel(source, index, slots, itemName, item) {
  ...
  const inMain =
    ...
    (mainRef?.index === index || mainRef?.name === itemName);
  const inOff =
    offRef?.source === source &&
    (offRef?.index === index || offRef?.name === itemName);
  const inArmor =
    source === "armor" &&
    armorRef &&
    (armorRef?.index === index || armorRef?.name === itemName);
  const inAccessory =
    source === "accessories" &&
    accessoryRef &&
    (accessoryRef?.index === index || accessoryRef?.name === itemName);
  const isTwoHand =
...
export default function PcEquipment({
  ...
  const { ensurePersonalPack, addItem: addCompendiumItem } =
    useCompendiumPacks();
  ...
  const patchInvCallback = useCallback(
    (source, updater) => {
  ...
  const handleEquipFull = useCallback(
    (source, index, item, isTwoHand, slotLabel, event) => {
      if (!setPlayer) return;
      if (slotLabel) {
        setPlayer((prev) => {
          let next = prev;
      ...
      if (source === "accessories") {
        setPlayer((prev) =>
          equipItemToSlot(prev, "accessory", {
            source,
            label: item?.name || "",
            index,
            item,
  ...
  const handleEquipToSlot = useCallback(
    (source, index, item, slot) => {
      if (!setPlayer) return;
      setPlayer((prev) =>
        equipItemToSlot(prev, slot, {
          source,
          label: item?.name || "",
          index,
          item,
        ...
        setPlayer((prev) => {
          ...
          return {
            ...
            equipment: prev?.equipment
              ? [{ ...eq0, customWeapons: updated }, ...prev.equipment.slice(1)]
              : [{ ...eq0, customWeapons: updated }],
          };
        });
      } else {
  ...
  const handleAddToCompendium = useCallback(
    async (source, item) => {
      const typeMap = {
        weapons: "weapon",
    ...
    () => [
      {
        key: "weapons",
        label: t("Weapons"),
        rows: (inv?.weapons || []).map((item, index) => ({
          item,
          source: "weapons",
          index,
        })),
      },
      {
        key: "customWeapons",
        label: t("Custom Weapons"),
        rows: (inv?.customWeapons || []).map((item, index) => ({
          item,
          source: "customWeapons",
          index,
        })),
      },
      {
        key: "shields",
        label: t("Shields"),
        rows: (inv?.shields || []).map((item, index) => ({
          item,
          source: "shields",
          index,
        })),
      ...
      },
      {
        key: "accessories",
        label: t("Accessories"),
        rows: (inv?.accessories || []).map((item, index) => ({
          item,
          source: "accessories",
          index,
        })),
        ...
        >
          ...
          <DialogActions sx={{ justifyContent: "space-between" }}>
            {isEditMode && setPlayer && (
              <Button
                onClick={() => openEditFromPreview(previewItem)}
                startIcon={<Edit />}
                variant="outlined"
              >
                {t("Edit")}
      ...
      {fullSections.map(({ key, label, rows }) => (
        <React.Fragment key={key}>
          <FullSectionHeader
            ...
            onCompendium={() =>
              setCompendiumType(
                key === "customWeapons" ? "custom-weapons" : key,
              )
            }
            primary={primary}
            t={t}
          ...
          {rows.length === 0 ? (
            ...
            rows.map((row) => {
              ...
              if (
                row.source === "customWeapons" &&
                isTransformingCustomWeapon(row.item)
              ) {
                return (
      ...
      >
        <MenuItem
          onClick={() => {
            if (slotMenuWeapon)
              setPlayer((prev) =>
                equipItemToSlot(prev, "mainHand", {
                  source: "weapons",
                  label: slotMenuWeapon.name,
                  index: slotMenuWeapon.index,
                  item: slotMenuWeapon.item,
                }),
              );
            setSlotMenuAnchor(null);
            setSlotMenuWeapon(null);
          ...
          onClick={() => {
            if (shieldMenuItem)
              setPlayer((prev) =>
                equipItemToSlot(prev, "mainHand", {
                  source: "shields",
                  label: shieldMenuItem.name,
                  index: shieldMenuItem.index,
                  item: shieldMenuItem.item,
                }),
              );
            setShieldMenuAnchor(null);
            setShieldMenuItem(null);
