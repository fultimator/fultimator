...
function renderSpellContent(spell, onUpdate, searchQuery, highlightMatchFn) {
  switch (spell.spellType) {
    ...
    case "invocation":
      return <SpellInvoker spell={spell} setPlayer={onUpdate} open={true} />;
    case "cooking":
      return <SpellGourmet spell={spell} open={true} />;
    case "magiseed":
      return <SpellMagiseed spell={spell} setPlayer={onUpdate} open={true} />;
    case "magichant":
    ...
    case "gift":
      return <SpellGift spell={spell} setPlayer={onUpdate} open={true} />;
    case "therioform":
      return <SpellTherioform spell={spell} />;
    case "pilot-vehicle":
      return <SpellVehicle spell={spell} />;
    case "deck":
      return <SpellDeck spell={spell} setPlayer={onUpdate} open={true} />;
    case "arcanist":
    case "arcanist-rework":
      return (
        <SpellArcanist
...
function buildBenefitLines(benefits, t) {
  ...
  if (benefits.martials?.armor)
    lines.push(t("Gain the ability to equip martial armor."));
  if (benefits.martials?.melee)
    lines.push(t("Gain the ability to equip martial melee weapons."));
  if (benefits.martials?.ranged)
    lines.push(t("Gain the ability to equip martial ranged weapons."));
  if (benefits.martials?.shields)
    lines.push(t("Gain the ability to equip martial shields."));
  ...
  return (
    <>
      <SectionSubHeader theme={theme}>
        ...
        >{`${t(clsName)} ${t("Free Benefits")}`}</Typography>
      </SectionSubHeader>
      <Box
...
function hasBenefits(benefits) {
  ...
  if (benefits.martials && Object.values(benefits.martials).some(Boolean))
    return true;
...
function SkillCard({
  ...
  const handleIncrement = () => {
    if (!onUpdate || skill.currentLvl >= skill.maxLvl || atClassLevelCap)
      return;
    onUpdate((prev) => ({
      ...prev,
        ...
        ci !== classIdx
          ...
          : {
              ...
              skills: c.skills.map((s, si) =>
                si === originalIdx ? { ...s, currentLvl: s.currentLvl + 1 } : s,
              ),
            },
      ),
    }));
      ...
      >
        ...
        <AccordionDetails sx={{ p: 0 }}>
          <Box sx={{ px: 2, pb: 0.75, pt: 0.5 }}>
            {renderSpellContent(spell, onUpdate, searchQuery, highlightMatch)}
          </Box>
        </AccordionDetails>
      </Accordion>
    );
...
function HeroicCard({
  ...
  const translatedDesc = cls.heroic?.description
          ...
          >
            ...
            <Typography
              sx={{
                ...
                fontSize: { xs: "1rem", sm: "1.1rem" },
                textTransform: "uppercase",
                color: hasHeroic ? "#fff" : "rgba(255,255,255,0.5)",
                flex: 1,
                lineHeight: 1.3,
              }}
            >
...
function SpellTypeAccordion({
  ...
  return (
    ...
    >
      <AccordionSummary
        expandIcon={<ExpandMore sx={{ color: "#fff", fontSize: "1.2rem" }} />}
        sx={{
          minHeight: 36,
          px: 2,
          py: 0,
...
function ClassSection({
  ...
  const showCompanionSection =
    Boolean(faithfulCompanionSkill) && allFaithfulSkills.length >= 1;
  ...
  useEffect(() => {
    ...
    getDocs(companionsQuery)
      .then((snap) =>
        setCompanionList(snap.docs.map((d) => ({ ...d.data(), id: d.id }))),
      )
  ...
  const filteredSkills = (cls.skills || [])
    ...
    .filter(({ skill }) => {
      ...
      return (
        t(skill.skillName).toLowerCase().includes(q) ||
        t(skill.description || "")
          .toLowerCase()
          .includes(q)
      );
  ...
  const filteredSpells = visibleSpells.filter((spell) => {
    if (!searchQuery?.trim()) return true;
    return getSpellSearchText(spell, t).includes(
  ...
  const hasContent =
    filteredSkills.length > 0 || filteredSpells.length > 0 || heroicVisible;
    ...
    return (
      <>
          ...
          >
            ...
            {onEditClass && (
              <Tooltip title={t("Edit Class")} arrow>
                <IconButton
                  size="small"
                  sx={{ p: "3px", color: "rgba(255,255,255,0.85)" }}
                  onClick={(e) => {
                    e.stopPropagation();
                    onEditClass(classIdx);
                  }}
                >
            ...
            <>
              ...
              >
                {filteredSkills.map(({ skill, originalIdx }) => (
                  <SkillCard
                    key={`skill-${classIdx}-${originalIdx}`}
                    skill={skill}
                    originalIdx={originalIdx}
                    classIdx={classIdx}
                    translatedDescription={t(skill.description || "")}
                    pc={pc}
                ...
                <Box sx={{ px: "10px", pb: "8px" }}>
                  ...
                  >
                    ...
                    >
                      ...
                      {cls.companion &&
                        ` - ${cls.companion.name} Lv ${cls.companion.lvl}`}
                    </Typography>
                    {isInteractive && (
                      <Button
                        size="small"
        ...
        >
          <DialogContent sx={{ p: 0 }}>
            {preview?.type === "skill" && (
              <SharedSkillCard
                item={{
                  ...preview.skill,
                  skillName: preview.skill.skillName,
                  description: preview.skill.description,
                  currentLvl: preview.skill.currentLvl,
                  className: cls.name,
                }}
              />
            )}
          ...
          <DialogActions>
            <Button onClick={() => setPreview(null)} variant="contained">
              {t("Close")}
            </Button>
          </DialogActions>
        </Dialog>
  ...
  return (
    <>
      <Paper
        elevation={3}
        sx={{ mb: 1.5, overflow: "hidden", borderRadius: "8px" }}
      >
        <Box
        ...
        >
          ...
          {isInteractive && onLevelChange ? (
            <Box
              ...
              onClick={(e) => e.stopPropagation()}
            >
              <Tooltip title={t("Decrease Level")} arrow>
                <span>
                  <IconButton
                    size="small"
                    sx={{
                    ...
                    disabled={cls.lvl <= 1}
                    onClick={() =>
                      onLevelChange(classIdx, Math.max(1, cls.lvl - 1))
                    }
                  >
              ...
              <Tooltip title={t("Increase Level")} arrow>
                <span>
                  <IconButton
                    size="small"
                    sx={{
                    ...
                    disabled={cls.lvl >= 10}
                    onClick={() =>
                      onLevelChange(classIdx, Math.min(10, cls.lvl + 1))
                    }
                  >
              ...
              {onRemoveClass && (
                <Tooltip title={t("Remove Class")} arrow>
                  <IconButton
                    ...
                    onClick={() => onRemoveClass(classIdx)}
                  >
            ...
            >
              ...
              <IconButton
                size="small"
                sx={{ p: "4px", color: "#fff", flexShrink: 0 }}
                onClick={(e) => {
                  e.stopPropagation();
              ...
              >
                {collapsed ? (
                  <KeyboardArrowDown sx={{ fontSize: "1.3rem" }} />
                ) : (
                  <KeyboardArrowUp sx={{ fontSize: "1.3rem" }} />
                )}
              </IconButton>
            </Box>
          )}
          ...
          <>
            ...
            <Box>
              ...
              {Object.entries(
                ...
                <SpellTypeAccordion
                  key={`spellgroup-${classIdx}-${spellType}`}
                  spellType={spellType}
                  spells={spells}
                  classIdx={classIdx}
                  onUpdate={onUpdate}
              ...
              {showCompanionSection && (
                <>
                  ...
                  <Box sx={{ px: 2, py: 1 }}>
                    {cls.companion ? (
                      <Typography
                        sx={{ fontFamily: "Antonio", fontSize: "1rem" }}
                      >
                        {cls.companion.name} - {t("Lvl")} {cls.companion.lvl}
                      </Typography>
                    ) : (
                    ...
                    {companionErr && (
                      <Typography variant="body2" color="error">
                        {t("Error Loading Companion List")}: {companionErr}
                      </Typography>
                    )}
                  </Box>
                </>
...
export default function PcClasses({
  ...
  const technospheresVariant =
    pc?.settings?.optionalRules?.technospheresVariant ?? "standard";
  ...
  const syncClassLevels = (next) =>
    automaticClassLevel ? syncAutomaticClassLevels(next) : next;

  const syncInnateClasses = (next) => {
    if (!usesInnateClassRules) return next;
    const innateClasses = (next.classes ?? [])
      .map((cls) => cls.name)
      .filter(Boolean)
      .slice(0, 3);
    return {
      ...
      settings: {
        ...next.settings,
        optionalRules: { ...next.settings?.optionalRules, innateClasses },
      },
    };
  };
  ...
  const addClassToPlayer = (
    ...
    const classExists = (pc?.classes ?? []).some(
    ...
    applyUpdate((prev) => ({
      ...prev,
      classes: [
        ...(Array.isArray(prev.classes) ? prev.classes : []),
        {
          name,
          fuid,
          lvl: 1,
  ...
  useEffect(() => {
    ...
    const idx = (pc?.classes ?? []).findIndex(
      (cls) =>
        cls?.name === pendingEditClassRef.name &&
        (cls?.fuid ?? null) === pendingEditClassRef.fuid,
    );
    if (idx < 0) return;
    setEditClassIdx(idx);
    setPendingEditClassRef(null);
  ...
  const handleLevelChange = (index, newLevel) => {
    applyUpdate((prev) => {
      const updatedClasses = prev.classes.map((cls, i) =>
        i === index ? { ...cls, lvl: newLevel } : cls,
      );
      if (newLevel === 10 && !updatedClasses[index].heroic)
        updatedClasses[index].heroic = { name: "", description: "" };
      const totalLevels = updatedClasses.reduce(
    ...
    applyUpdate((prev) => ({
      ...
      classes: prev.classes.map((cls, i) => {
        ...
        const mergedSkills = patch.skills.map((ps) => {
          const existing = cls.skills?.find(
            (s) => s.skillName === ps.skillName,
          );
          return existing
            ? { ...ps, currentLvl: existing.currentLvl }
            : { ...ps, currentLvl: 0 };
        });
  ...
  const {
    ...
    isInteractive && onUpdate ? pc : null,
    isInteractive && onUpdate ? (updater) => onUpdate(updater) : () => {},
  );
  ...
  const warnings = (() => {
    ...
    if (usesInnateClassRules) {
      ...
      const innateTotal = pc.classes
        ? pc.classes.reduce((s, c) => s + parseInt(c.lvl), 0)
        : 0;
      const mnemoTotal = pc.info?.mnemoLevelsSpent ?? 0;
      if (innateTotal + mnemoTotal > pc.lvl)
        w.push(
          "Sum of innate class levels and invested mnemosphere levels exceeds character level.",
        );
    } else {
      if (!pc.classes || pc.classes.length < 2)
        w.push("Character must have at least 2 classes.");
      const maxLvlCount = pc.classes
        ? pc.classes.filter((c) => c.lvl >= 10).length
        : 0;
      if (pc.classes && pc.classes.length - maxLvlCount > 3)
        w.push(
          "The number of classes exceeds the limit beyond the number of classes at level 10.",
        );
      const total = pc.classes
        ? pc.classes.reduce((s, c) => s + parseInt(c.lvl), 0)
        : 0;
      if (total !== pc.lvl)
  ...
  const totalInvested = (pc?.classes ?? []).reduce(
    (s, c) => s + (parseInt(c.lvl) || 0),
    0,
  );
  const characterLevel = pc?.lvl ?? 0;
  const totalMnemoInvested = pc?.info?.mnemoLevelsSpent ?? 0;
  const editingClass =
    editClassIdx !== null ? pc?.classes?.[editClassIdx] : null;
  ...
  const filteredClasses = (pc?.classes ?? []).filter((cls) => {
    ...
    return (
      ...
      cls.skills?.some(
        (s) =>
          t(s.skillName).toLowerCase().includes(q) ||
          t(s.description || "")
            .toLowerCase()
            .includes(q),
      ) ||
  ...
  const classesActions = (
    <Box
      sx={{ display: "flex", alignItems: "center", gap: isCompact ? 0.5 : 1 }}
    >
      {isInteractive && (
        <>
      ...
      >
        {[
          ...
          usesInnateClassRules
            ? characterLevel - totalMnemoInvested
            : characterLevel,
        ].map((val, i) => (
      ...
      <Tooltip
        title={
          expandSignal?.expanded
            ? t("Collapse All Classes")
            : t("Expand All Classes")
        }
      >
        <IconButton
          size="small"
          data-expand-all-classes={
            expandSignal?.expanded ? "expanded" : "collapsed"
          }
          onClick={(e) => {
            e.stopPropagation();
            setExpandSignal((v) => ({
