...
export default function PlayerCampActivities({ player, isEditMode = false, onEdit, speaker = "", compact = false, headerActions }) {
  const { t } = useTranslate();
  ...
  if (compact) {
    return (
      <Paper sx={{ mb: 1, overflow: "hidden" }} elevation={0} variant="outlined">
        <CompactSectionHeader title={t("Camp Activities (Max 2)")}>{headerActions}</CompactSectionHeader>
        {list}
      </Paper>
    );
  }
  ...
  return (
    <SectionCard title={t("Camp Activities (Max 2)")} noShadow sx={{ mb: 1 }} actions={headerActions}>
      {list}
    </SectionCard>
  );
}
