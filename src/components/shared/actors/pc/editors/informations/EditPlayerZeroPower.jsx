...
function fromFormState(form) {
  return {
    ...
    zeroTrigger: {
      ...
      description: form.triggerDescription ?? "",
    },
    zeroEffect: {
      name: form.effectName ?? "",
      description: form.effectDescription ?? "",
    },
    clock: { sections: Number(form.clockSections) || 6 },
  };
}

export default function EditPlayerZeroPower({
  player,
  setPlayer,
  isEditMode,
  externalOpen = false,
  ...
  const hasDetails =
    zeroPower?.zeroTrigger?.description || zeroPower?.zeroEffect?.description;
        ...
        {editorOpen && (
          <ItemEditModal
            ...
            onSave={(payload) => {
              setPlayer((prev) => ({
                ...prev,
                zeroPower: fromFormState(payload),
              }));
              setEditorOpen(false);
              setCreating(false);
              onExternalClose?.();
        ...
        <Box sx={{ p: 0.75 }}>
          {!hasZeroPower ? (
            ...
            <ItemRowCard
              variant="outlined"
              onCardClick={
                hasDetails ? () => setExpanded((v) => !v) : undefined
              }
              ...
              {expanded && (
                ...
                >
                  <SharedZeroPowerCard
                    item={{
                      ...zeroPower,
                      clock: zeroPower.clock?.sections || 6,
                    }}
                  />
                </Box>
              )}
